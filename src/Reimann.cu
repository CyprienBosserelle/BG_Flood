...
/**
* @brief "Adaptive" second-order hydrostatic reconstruction. GPU version for the X-axis
 *
 * ## Description
 ...
 * This reconstruction is safe for steep slopes with thin water depth and is well-balanced, conserving "lake-at-rest" states.
 *
 ...
 * 
 * @param XParam Structure containing the simulation parameters
 * @param XBlock Structure containing the block information
 * @param XEv Structure containing the evolving variables
 * @param XGrad Structure containing the gradients of the evolving variables
 * @param XFlux Structure containing the fluxes to be updated
 * @param dtmax Array to store the maximum allowable time step for each cell
 * @param zb Array containing the bed elevation
 * @param T Data type, either float or double
 * @note This function is designed to be run on the GPU and should be launched with a grid and block configuration that matches the number of blocks and block size.
 */
 ...
 * @param dtmax Array to store the maximum allowable time step for each cell
 * @param zb Array containing the bed elevation
 * @tparam T Data type, either float or double
 * @note This function is designed to be run on the CPU and should be called within a loop over all blocks.
 * 
 */
 ...
 * @param delta Grid resolution at the current level
 * @param epsi Small number to prevent division by zero
 * @param CFL Courant-Friedrichs-Lewy number for stability condition
 * @param cm Metric term for spherical coordinates (1.0 for Cartesian)
 * @param fm Metric term for spherical coordinates (1.0 for Cartesian)
 * @param hm Water depth on the left side of the interface
 * @param hp Water depth on the right side of the interface
 * @param um Velocity in the x-direction on the left side of the interface
 * @param up Velocity in the x-direction on the right side of the interface
 * @param fh Reference to store the computed flux for water depth
 * @param fq Reference to store the computed flux for momentum
 * @return The maximum allowable time step based on the wave speeds and CFL condition
 * @tparam T Data type, either float or double
 * @note This function is designed to be run on both CPU and GPU.
 *
*/
