...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{Manual_autotoc_md107}{}\doxysection{Input Parameters}\label{Manual_autotoc_md107}
\hypertarget{Manual_autotoc_md108}{}\doxysubsection{$<$tt$>$\+BG\+\_\+param.\+txt$<$/tt$>$}\label{Manual_autotoc_md108}
All the model inputs are controlled by the BG\+\_\+param.\+txt file. It is a simple text file that contains the parameters of the model that the user wishes to change.\hypertarget{Manual_autotoc_md109}{}\doxysubsubsection{How to use the $<$tt$>$\+BG\+\_\+param.\+txt$<$/tt$>$ file}\label{Manual_autotoc_md109}
...
\end{DoxyCode}
 \hypertarget{Manual_autotoc_md110}{}\doxysubsubsection{General comment about input files}\label{Manual_autotoc_md110}
...
Where the file input is a timeserie (e.\+g. boundary forcing), then the file extension is ignored but a text file is expected.\hypertarget{Manual_autotoc_md111}{}\doxysubsection{List of Parameters}\label{Manual_autotoc_md111}
Remember that the only required parameter to run the model is the bathymetry file. But if you want to run something fun you will need to specify boundary conditions, initial conditions and/or some forcings.\hypertarget{Manual_autotoc_md112}{}\doxysubsubsection{Input}\label{Manual_autotoc_md112}
\tabulinesep=1mm
...
\end{longtabu}
\hypertarget{Manual_autotoc_md113}{}\doxysubsubsection{Forcing/\+Boundary}\label{Manual_autotoc_md113}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md114}{}\doxysubsubsection{Hydrodynamics}\label{Manual_autotoc_md114}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md115}{}\doxysubsubsection{Time keeping}\label{Manual_autotoc_md115}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md116}{}\doxysubsubsection{Output}\label{Manual_autotoc_md116}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md117}{}\doxysubsubsection{Miscelanious}\label{Manual_autotoc_md117}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md118}{}\doxysubsection{Bathymetry/topography files}\label{Manual_autotoc_md118}
...
This input file is critical as it defines the extent of the model grid and the base resolution in the model. (The model is not adaptive yet but this is still relevant for the extent of the model.)\hypertarget{Manual_autotoc_md119}{}\doxysubsection{Conputational mesh}\label{Manual_autotoc_md119}
BG-\/\+Flood generates its own mesh. By default, it is a quad regular mesh based on the DEM (Digital Elevation \mbox{\hyperlink{structModel}{Model}}) extend and resolution. The extend of the mesh and resolution of the mesh can be modify by the user. The mesh can also be refined/coarsen in areas or following patterns prescribed by the user.\hypertarget{Manual_autotoc_md120}{}\doxysubsubsection{Adaptative mesh}\label{Manual_autotoc_md120}
\hypertarget{Manual_autotoc_md121}{}\doxysubsubsection{Masking}\label{Manual_autotoc_md121}
...
\end{DoxyCode}
\hypertarget{Manual_autotoc_md123}{}\doxysubsubsection{Boundary file (for type 2 or 3)}\label{Manual_autotoc_md123}
Water level boundary file are needed to type 2 nd 3 boundaries. The files are 2 (or more) column one with time in the first column and water level is the other(s). Note that the time step in the file doesn\textquotesingle{}t need to be constant. The model will linearly interpolated between steps in the file. The file can be either comma separated or tab separated. This is automatically detected.\hypertarget{Manual_autotoc_md124}{}\doxyparagraph{Uniform boundary file}\label{Manual_autotoc_md124}
...
\end{DoxyCode}
\hypertarget{Manual_autotoc_md125}{}\doxyparagraph{Variable boundary file}\label{Manual_autotoc_md125}
...
In this case both near and far end of the boundary axis will remain zero and the center of the boundary axis will be 1.\+2m.\hypertarget{Manual_autotoc_md126}{}\doxysubsection{Bottom friction}\label{Manual_autotoc_md126}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md135}{}\doxysubsubsection{Mean/averaged output between output steps}\label{Manual_autotoc_md135}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md136}{}\doxysubsubsection{Max output}\label{Manual_autotoc_md136}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md137}{}\doxysubsubsection{Risk assesment related output}\label{Manual_autotoc_md137}
...
zsmax   &\PBS\centering Maximum Water level elevation above datum   &\PBS\raggedleft \mbox{[}m\mbox{]}   \\\cline{1-3}
\end{longtabu}
\hypertarget{Manual_autotoc_md138}{}\doxysubsubsection{Infiltration outputs}\label{Manual_autotoc_md138}
...
zsmax   &\PBS\centering Maximum Water level elevation above datum   &\PBS\raggedleft \mbox{[}m\mbox{]}   \\\cline{1-3}
\end{longtabu}
\hypertarget{Manual_autotoc_md139}{}\doxysubsubsection{Other gradients and intermediate terms of the equations}\label{Manual_autotoc_md139}
