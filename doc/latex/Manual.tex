...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{Manual_autotoc_md103}{}\doxysection{Input Parameters}\label{Manual_autotoc_md103}
\hypertarget{Manual_autotoc_md104}{}\doxysubsection{$<$tt$>$\+BG\+\_\+param.\+txt$<$/tt$>$}\label{Manual_autotoc_md104}
All the model inputs are controlled by the BG\+\_\+param.\+txt file. It is a simple text file that contains the parameters of the model that the user wishes to change.\hypertarget{Manual_autotoc_md105}{}\doxysubsubsection{How to use the $<$tt$>$\+BG\+\_\+param.\+txt$<$/tt$>$ file}\label{Manual_autotoc_md105}
...
\end{DoxyCode}
 \hypertarget{Manual_autotoc_md106}{}\doxysubsubsection{General comment about input files}\label{Manual_autotoc_md106}
...
Where the file input is a timeserie (e.\+g. boundary forcing), then the file extension is ignored but a text file is expected.\hypertarget{Manual_autotoc_md107}{}\doxysubsection{List of Parameters}\label{Manual_autotoc_md107}
Remember that the only required parameter to run the model is the bathymetry file. But if you want to run something fun you will need to specify boundary conditions, initial conditions and/or some forcings.\hypertarget{Manual_autotoc_md108}{}\doxysubsubsection{Input}\label{Manual_autotoc_md108}
\tabulinesep=1mm
...
\end{longtabu}
\hypertarget{Manual_autotoc_md109}{}\doxysubsubsection{Forcing/\+Boundary}\label{Manual_autotoc_md109}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md110}{}\doxysubsubsection{Hydrodynamics}\label{Manual_autotoc_md110}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md111}{}\doxysubsubsection{Time keeping}\label{Manual_autotoc_md111}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md112}{}\doxysubsubsection{Output}\label{Manual_autotoc_md112}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md113}{}\doxysubsubsection{Miscelanious}\label{Manual_autotoc_md113}
\tabulinesep=1mm
\begin{longtabu}spread 0pt [c]{*{3}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Parameter (synonyms)   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Definition   }&\PBS\raggedleft \cellcolor{\tableheadbgcolor}\textbf{ Default Value    }\\\cline{1-3}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md114}{}\doxysubsection{Bathymetry/topography files}\label{Manual_autotoc_md114}
...
This input file is critical as it defines the extent of the model grid and the base resolution in the model. (The model is not adaptive yet but this is still relevant for the extent of the model.)\hypertarget{Manual_autotoc_md115}{}\doxysubsection{Conputational mesh}\label{Manual_autotoc_md115}
BG-\/\+Flood generates its own mesh. By default, it is a quad regular mesh based on the DEM (Digital Elevation \mbox{\hyperlink{structModel}{Model}}) extend and resolution. The extend of the mesh and resolution of the mesh can be modify by the user. The mesh can also be refined/coarsen in areas or following patterns prescribed by the user.\hypertarget{Manual_autotoc_md116}{}\doxysubsubsection{Adaptative mesh}\label{Manual_autotoc_md116}
\hypertarget{Manual_autotoc_md117}{}\doxysubsubsection{Masking}\label{Manual_autotoc_md117}
...
\end{DoxyCode}
\hypertarget{Manual_autotoc_md119}{}\doxysubsubsection{Boundary file (for type 2 or 3)}\label{Manual_autotoc_md119}
Water level boundary file are needed to type 2 nd 3 boundaries. The files are 2 (or more) column one with time in the first column and water level is the other(s). Note that the time step in the file doesn\textquotesingle{}t need to be constant. The model will linearly interpolated between steps in the file. The file can be either comma separated or tab separated. This is automatically detected.\hypertarget{Manual_autotoc_md120}{}\doxyparagraph{Uniform boundary file}\label{Manual_autotoc_md120}
...
\end{DoxyCode}
\hypertarget{Manual_autotoc_md121}{}\doxyparagraph{Variable boundary file}\label{Manual_autotoc_md121}
...
In this case both near and far end of the boundary axis will remain zero and the center of the boundary axis will be 1.\+2m.\hypertarget{Manual_autotoc_md122}{}\doxysubsection{Bottom friction}\label{Manual_autotoc_md122}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md131}{}\doxysubsubsection{Mean/averaged output between output steps}\label{Manual_autotoc_md131}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md132}{}\doxysubsubsection{Max output}\label{Manual_autotoc_md132}
...
\end{longtabu}
\hypertarget{Manual_autotoc_md133}{}\doxysubsubsection{Risk assesment related output}\label{Manual_autotoc_md133}
...
zsmax   &\PBS\centering Maximum Water level elevation above datum   &\PBS\raggedleft \mbox{[}m\mbox{]}   \\\cline{1-3}
\end{longtabu}
\hypertarget{Manual_autotoc_md134}{}\doxysubsubsection{Infiltration outputs}\label{Manual_autotoc_md134}
...
zsmax   &\PBS\centering Maximum Water level elevation above datum   &\PBS\raggedleft \mbox{[}m\mbox{]}   \\\cline{1-3}
\end{longtabu}
\hypertarget{Manual_autotoc_md135}{}\doxysubsubsection{Other gradients and intermediate terms of the equations}\label{Manual_autotoc_md135}
